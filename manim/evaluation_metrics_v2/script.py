...
class Scene1_ClassificationMetrics(SafeScene):
    def construct(self):
        ...
        # Create dataset inherently split by Y-axis for Actuals
        np.random.seed(42)
        # Actual Positives (Green) in upper half
        pos_dots = VGroup(*[Dot(color=SECONDARY).move_to([np.random.uniform(-3.5, 3.5), np.random.uniform(0.5, 2.5), 0]) for _ in range(20)])
        # Actual Negatives (Red) in lower half
        neg_dots = VGroup(*[Dot(color=ACCENT).move_to([np.random.uniform(-3.5, 3.5), np.random.uniform(-2.5, -0.5), 0]) for _ in range(20)])
        ...
        # We classify without drawing the rigid lines
        self.wait(1)
        
        self.add_subcaption("This splits the data into 4 groups: True Positives, False Positives...", duration=4)
        
        col_label_1 = Text("Predicted -", font_size=24, color=YELLOW).move_to([-2.5, 3.2, 0])
        col_label_2 = Text("Predicted +", font_size=24, color=YELLOW).move_to([2.5, 3.2, 0])
        row_label_1 = Text("Actual +", font_size=24, color=SECONDARY).move_to([-4.5, 1.5, 0])
        row_label_2 = Text("Actual -", font_size=24, color=ACCENT).move_to([-4.5, -1.5, 0])
        
        grid_labels = VGroup(col_label_1, col_label_2, row_label_1, row_label_2)
        self.play(FadeIn(grid_labels))
        
        # Classify based on position
        tp_dots = VGroup(*[d for d in pos_dots if d.get_x() > 0])
        fn_dots = VGroup(*[d for d in pos_dots if d.get_x() <= 0])
        fp_dots = VGroup(*[d for d in neg_dots if d.get_x() > 0])
        tn_dots = VGroup(*[d for d in neg_dots if d.get_x() <= 0])
        
        labels = VGroup(
            Text("True Positives", font_size=24).move_to([2.5, 1.5, 0]),
            Text("False Negatives", font_size=24).move_to([-2.5, 1.5, 0]),
            Text("False Positives", font_size=24).move_to([2.5, -1.5, 0]),
            Text("True Negatives", font_size=24).move_to([-2.5, -1.5, 0])
        )
        ...
        # Manually construct fraction to highlight parts
        prec_title = Text("Precision = ", font_size=36)
        prec_num = Text("TP", font_size=30, color=SECONDARY)
        prec_line = Line(LEFT, RIGHT).set_length(1.5)
        prec_den = Text("TP + FP", font_size=30, color=YELLOW)
        prec_frac = VGroup(prec_num, prec_line, prec_den).arrange(DOWN, buff=0.2)
        prec_eq = VGroup(prec_title, prec_frac).arrange(RIGHT, buff=0.2).to_edge(DOWN)
        
        self.play(Write(prec_title), Create(prec_line))
        
        # Highlight Numerator (TP)
        self.play(Write(prec_num), Indicate(tp_dots, scale_factor=2.0, color=SECONDARY))
        self.wait(0.5)
        
        # Highlight Denominator (TP + FP)
        ...
        rec_title = Text("Recall = ", font_size=36)
        rec_num = Text("TP", font_size=30, color=SECONDARY)
        rec_line = Line(LEFT, RIGHT).set_length(1.5)
        rec_den = Text("TP + FN", font_size=30, color=YELLOW)
        rec_frac = VGroup(rec_num, rec_line, rec_den).arrange(DOWN, buff=0.2)
        rec_eq = VGroup(rec_title, rec_frac).arrange(RIGHT, buff=0.2).to_edge(DOWN)
        ...
        # Highlight Denominator (TP + FN)
        self.play(Write(rec_den), Indicate(VGroup(tp_dots, fn_dots), scale_factor=2.0, color=YELLOW))
        self.wait(2)
        
        self.play(FadeOut(VGroup(rec_eq, all_dots, labels, grid_labels, title)))
...
class Scene2_CurvesAndThresholds(SafeScene):
    def construct(self):
        ...
        # 1D line for probabilities
        number_line = NumberLine(x_range=[0, 1, 0.2], length=6).shift(DOWN*2.5 + LEFT*2)
        line_label = Text("Probability", font_size=20).next_to(number_line, LEFT)
        ...
        dots = VGroup()
        for p in pos_probs:
            dots.add(Dot(number_line.n2p(p), color=SECONDARY).shift(UP*0.2))
        for p in neg_probs:
            dots.add(Dot(number_line.n2p(p), color=ACCENT).shift(UP*0.2))
        ...
        t_label = always_redraw(lambda: DecimalNumber(threshold.get_value(), num_decimal_places=2, font_size=20).next_to(t_line, UP))
        
        self.play(Create(t_line), Write(t_label))
        
        # Updaters for dot coloring (dim if below threshold)
        ...
        # --- ROC Curve ---
        axes_roc = Axes(x_range=[0, 1.1, 0.2], y_range=[0, 1.1, 0.2], x_length=4, y_length=4, axis_config={"include_numbers": False}).to_edge(RIGHT, buff=1).shift(UP*0.5)
        roc_x_label = Text("FPR", font_size=20).next_to(axes_roc.x_axis, DOWN)
        roc_y_label = Text("TPR", font_size=20).next_to(axes_roc.y_axis, LEFT).rotate(PI/2)
        ...
        auc_text = Text("AUC = 0.94", font_size=24, color=YELLOW).move_to(axes_roc.c2p(0.6, 0.4))
        ...
        # Fade out ROC stuff and reset threshold
        self.play(
            FadeOut(VGroup(roc_path, auc_poly, auc_text, roc_dot, roc_x_label, roc_y_label)), 
            threshold.animate.set_value(1.1)
        )
        
        pr_x_label = Text("Recall", font_size=20).next_to(axes_roc.x_axis, DOWN)
        pr_y_label = Text("Precision", font_size=20).next_to(axes_roc.y_axis, LEFT).rotate(PI/2)
        ...
        ap_text = Text("AP", font_size=24, color=YELLOW).move_to(axes_roc.c2p(0.5, 0.5))
...
class Scene3_VarianceAndVectors(SafeScene):
    def construct(self):
        ...
        axes = Axes(x_range=[0, 10], y_range=[0, 10], x_length=6, y_length=5).shift(DOWN*0.5 + LEFT*1)
        self.play(Create(axes))
        
        # 1. Organic Data Generation
        np.random.seed(42)
        ...
        # We will use a ValueTracker for the slope (m) and intercept (b)
        ...
        # Dynamic Squares
        # We need an updater that draws a square from each dot to the line
        def get_squares():
            squares = VGroup()
            ...
            for x, y in zip(x_vals, y_vals):
                ...
                if side_len > 0.01:
                    ...
                    sq.move_to(p_dot)
                    # Align the TOP of the square to the higher of the two points
                    higher_point = p_dot if y > y_pred else p_line
                    sq.align_to(higher_point, UP)
                    # Align the LEFT of the square to the x-coordinate
        ...
        var_eq = MathTex(r"\text{Variance} = \frac{1}{n} \sum (y_i - \bar{y})^2", font_size=36).to_edge(RIGHT, buff=0.5).shift(UP*1.5)
        ...
        mse_eq = MathTex(r"\text{MSE} = \frac{1}{n} \sum (y_i - \hat{y}_i)^2", font_size=36).next_to(var_eq, DOWN, buff=0.7)
        
        # Dynamically fit the line! The squares will shrink organically.
        self.play(
        ...
        r2_eq = MathTex(r"R^2 = 1 - \frac{\text{MSE}}{\text{Variance}}", font_size=36).next_to(mse_eq, DOWN, buff=1)
        self.play(Write(r2_eq))
        self.wait(3)
        
        self.play(FadeOut(VGroup(title, axes, dots, fit_line, squares_group, var_eq, mse_eq, r2_eq)))
...
class Scene4_ObjectDetection(SafeScene):
    def construct(self):
        ...
        gt_box = Rectangle(width=4, height=3, color=SECONDARY, stroke_width=4).move_to(ORIGIN)
        gt_label = Text("Ground Truth", color=SECONDARY, font_size=24).next_to(gt_box, UP)
        
        pred_box = Rectangle(width=3, height=2.5, color=ACCENT, stroke_width=4).move_to(LEFT*3 + DOWN*1)
        pred_label = always_redraw(lambda: Text("Prediction", color=ACCENT, font_size=24).next_to(pred_box, DOWN))
        ...
        # We simulate Intersection
        # Intersection Mobject can be tricky with updaters if shapes are completely disjoint, so we handle it gracefully.
        def get_iou_value():
        ...
        iou_text = always_redraw(lambda: Text(f"IoU: {get_iou_value():.2f}", font_size=36).to_edge(RIGHT).shift(UP*1))
        self.play(FadeIn(iou_text))
        
        # Visual Intersection polygon
        ...
        self.play(pred_box.animate.move_to(gt_box.get_center() + RIGHT*0.5 + UP*0.2), run_time=4)
        self.wait(1)
        ...
        self.play(FadeOut(VGroup(title, gt_box, gt_label, pred_box, pred_label, inter_poly, iou_text, map_text)))
